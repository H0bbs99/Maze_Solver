import time
import random
from graphic import Line, Point, Window
...
class Maze():
    ...
    def _solve_r(self, i, j):
        ...
        if i < self._num_cols - 1 and not self._cells[i + 1][j].visited and not self._cells[i][j].has_right_wall:
            line_id = self._win.canvas.create_line(
                self._x1 + i * self._cell_size_x + self._cell_size_x // 2,
                self._y1 + j * self._cell_size_y + self._cell_size_y // 2,
                self._x1 + (i + 1) * self._cell_size_x + self._cell_size_x // 2,
                self._y1 + j * self._cell_size_y + self._cell_size_y // 2,
                fill="blue"
            )
            if self._solve_r(i + 1, j):
                return True
            self._win.canvas.delete(line_id)
        ...
        if j < self._num_rows - 1 and not self._cells[i][j + 1].visited and not self._cells[i][j].has_bottom_wall:
            line_id = self._win.canvas.create_line(
                ...
                self._y1 + j * self._cell_size_y + self._cell_size_y // 2,
                self._x1 + i * self._cell_size_x + self._cell_size_x // 2,
                self._y1 + (j + 1) * self._cell_size_y + self._cell_size_y // 2,
                fill="blue"
            )
